...
function gameOver() {
  ...
  const savedMaxTime = localStorage.getItem("maxTime") || "00:00";
  const minutes = parseInt(currentTime.split(":")[0]);
  const seconds = parseInt(currentTime.split(":")[1]);
  const currentSeconds = minutes * 60 + seconds;
  const savedMinutes = parseInt(savedMaxTime.split(":")[0]);
  const savedSeconds = parseInt(savedMaxTime.split(":")[1]);
  const savedMaxSeconds = savedMinutes * 60 + savedSeconds;
  if (currentSeconds > savedMaxSeconds) {
    localStorage.setItem("maxTime", currentTime);
    document.getElementById("maxtime").innerText = currentTime;
  }
  else {
    document.getElementById("maxtime").innerText = savedMaxTime;
  }
  ...
  if (currentLevel > savedMaxLevel) {
    ...
    document.getElementById("maxlevel").innerText = currentLevel;
  }
  else {
    document.getElementById("maxlevel").innerText = savedMaxLevel;
  }
}
...
function updateTimer() {
  ...
  // Check if the current time elapsed is greater than the previous max time
  const prevMaxTime = localStorage.getItem("maxTime");
  if (prevMaxTime === null || timeElapsed > convertTimeToSeconds(prevMaxTime)) {
    // If so, update the max time in local storage and on the page
    maxTime = formattedMinutes + ':' + formattedSeconds;
    localStorage.setItem("maxTime", maxTime);
    document.getElementById("maxtime").innerText = maxTime;
  }
}

// Helper function to convert time in the format "mm:ss" to seconds
